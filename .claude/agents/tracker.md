...
Read `.claude/playbooks/tracker.md` first if it exists. It **appends** to this
...
with an explicit `fields=`. CLIs only — no MCP servers: you choose the fields,
the output pipes, and nothing loads a schema. Resolve indirection yourself —
"what's blocking this PR" returns the blocking thing.
